...
class District(Base):
    """A coordinator's operating area (mirrors the Plus-Codes-style village/
    post-office/police-station chain used in the report rather than a formal
    street address)."""
...
class Household(Base):
    """A single household pre-mapped by the local volunteer team, with the
    vulnerability flags the report identifies as the missing prioritisation
    signal (elderly-only, no resident smartphone, low-lying position)."""
    ...
    landmark_chain = Column(String, nullable=False)  # e.g. "near Shiv Mandir, Ward 4"
...
class Alert(Base):
    """A flood alert for a district. In production this would be ingested from
    Google Flood Hub's public feed for the matching river gauge; here it's
    generated by a deterministic simulator so the project is fully demoable
    without needing gauge-level API access (see app/flood_feed.py)."""
...
class ChecklistItem(Base):
    """One row of the ordered door-to-door checklist generated for an alert.
    Stored (not recomputed on every read) so a volunteer's progress persists
    even if the alert's priority ranking would later change."""
    ...
    status = Column(String, default="pending")  # pending | warned | unreachable
